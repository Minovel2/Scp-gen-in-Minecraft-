...
system.runInterval(() => {
        ...
        for (let i=0;i<ent.length;i++) {
            if (ent[i].id == "map:helper" && ent[i].nameTag.split("-")[0] == "set") {
                ...
                try {
                ent[i].runCommandAsync(`setblock ~~~ ${spl[1]} ${data}`);
                } catch {}
                ent[i].kill();
            }
        }
...
function place() {
    ...
    for (let j=0;j<100;j++) {
        ...
        if (docking[j]) {
            try {
           if (
              over.runCommand(`structure load ${rooms[floorplan[j]]}_${num[docking[j]-1]} ${centreX + (startX -x1)*26} ${z1} ${centreY + (startY-y1)*26} ${degr[docking[j]-1]*90}_degrees`).successCount == 0
